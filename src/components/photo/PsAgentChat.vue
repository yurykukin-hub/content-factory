...
/**
 * AI Agent chat for Photo Studio.
 * Pattern from SsAgentChat.vue, adapted for photo generation context.
 */
import { ref, computed, watch, nextTick } from 'vue'
import { SendHorizontal, ChevronDown, Loader2, Bot, Sparkles, Camera, Mic, Square } from 'lucide-vue-next'
  ...
  <div class="flex flex-col flex-1 min-h-0">
    ...
    <div class="flex items-center justify-between px-4 py-2 border-b border-gray-100 dark:border-gray-800">
      <div class="flex items-center gap-2">
        <Camera :size="13" class="text-fuchsia-500" />
        <span class="text-[11px] font-medium text-gray-500">AI-агент фото-студии</span>
      </div>
      <div class="relative">
        ...
        <div v-if="showModeMenu"
          ...
          <button @click="emit('update:mode', 'simple'); showModeMenu = false"
            ...
            Простой
            <span class="block text-[10px] text-gray-400 mt-0.5">Быстрый промпт, авто-настройки</span>
          </button>
          <button @click="emit('update:mode', 'advanced'); showModeMenu = false"
            :class="[mode === 'advanced' ? 'text-fuchsia-600 font-medium' : 'text-gray-600']"
            class="w-full text-left px-3 py-2 text-xs hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors">
            Продвинутый
            <span class="block text-[10px] text-gray-400 mt-0.5">Детальный контроль, варианты</span>
          </button>
        </div>
      </div>
    </div>
    ...
    <div ref="messagesContainer" class="flex-1 overflow-y-auto px-2 py-2 space-y-2 lg:px-4 lg:py-3 lg:space-y-3">
      ...
      <div v-if="!messages.length && !loading" class="flex gap-2 mb-3">
        ...
        <div class="bg-gray-100 dark:bg-gray-800 px-3 py-2 rounded-2xl rounded-bl-md text-sm text-gray-700 dark:text-gray-300 leading-relaxed">
          <p v-if="contextSummary">{{ contextSummary }}</p>
          <p class="mt-1.5 text-gray-500">Опиши, какое изображение хочешь создать</p>
        </div>
      </div>
    ...
    <div class="px-2 pb-2 lg:px-4 lg:pb-3">
      <div class="flex gap-2 items-end">
        <textarea ref="textareaRef" v-model="inputText" @keydown="onKeydown"
          :placeholder="mode === 'simple' ? 'Опиши изображение в двух словах...' : 'Подробно опиши, какое фото хочешь...'"
