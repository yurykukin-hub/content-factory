...
## Ключевые факты
- [2026-06-04] **SMM-аналитика (Эпик B Phase 4+5) — в проде.** Миграция `20260604185535_add_smm_analytics` (4 модели: SocialPostMetricSnapshot append-only, SocialAccountMetricSnapshot, SiteTrafficSnapshot, AnalyticsReport). `services/analytics/` (типы+vk/postmypost/metrika адаптеры+collector+analyst-agent+analyst-telegram), `metrics-poller.ts` (2×/день через scheduler, `metrics_times_utc` default 05:00,15:00), `routes/analytics.ts` (collect/overview/report/reports), `AnalyticsView.vue` (/analytics). Verified live: IG 9 снапшотов, отчёт НаWоде (ER 5.18%). Коммиты 669140d+6e9cc3f. ADR `decisions/2026-06-smm-analytics-architecture` → accepted. Включить недельный агент: AppConfig `analytics_agent_enabled=true`.
- [2026-06-04] **Postmypost analytics — ключи сняты вживую:** `/analytics/publications?project_id&account_id&date_from&date_to&per_page&page&type` → `analytics:{likes,shares,engagements,views,reach,engagement_rate_reach,engagement_rate_views}` + поля `id`(pm),`external_id`(IG media),`external_url`,`type`(1=post/3=story/4=reels). **Джойн к CF:** `externalPostId`(pm pub id) → `GET /publications/{id}` → `posts[0].external_id` + реальный IG-url → матч с analytics-строкой. `/analytics/accounts` требует ещё `metrics` (коды не сняты — account-level IG отложен).
- [2026-06-04] **VK-метрики:** `wall.getById` требует **USER-токен** (community/per-channel → error 27, как и wall-photos); читать через `ensureValidToken()` (авто-рефреш), НЕ `PlatformAccount.accessToken` (там community NAWODE). `stats.get`/`stories.getStats` → scope `stats` (добавлен в OAuth `vk-oauth.ts`, нужна ре-авторизация; сейчас code 15/100). **CF VK-контент = СТОРИС** (id `story-...`), статы живут ~24ч → поллер ловит в день публикации; `wall.getById` — только для обычных постов (их у НаWоде в CF пока нет). Метрика-счётчик nawode.ru = **92916147**.
- [2026-06-04] **ИНЦИДЕНТ:** `docker compose up` из `/home/dev/projects/content-factory` уронил прод-БД (dev и prod = один compose-проект `content-factory` по basename → общие сети/volumes; пересоздал postgres на сети `_default`, прод-бэкенд на `_internal` потерял БД). Восстановлено (rm dev-контейнер → prod compose up → restart backend), данные целы. **Правило:** локальную dev-БД — ТОЛЬКО standalone (`docker run --name cf-dev-pg -p 5441:5432 postgres:16-alpine`, без compose-лейблов); НИКОГДА не `compose up/down` из dev-папки. Детали: [[incidents/2026-06]].
- [2026-04-05] VK фото-постинг: Community Token limitation (error 27), нужен User Token через OAuth
- [2026-06-04] VK фото на СТЕНУ — диагностика (НаWоде): community-токен → **error 27** ("method unavailable with group auth"); app-level user-токен (vk_user_token) → **error 15** ("no access... current scopes") = у токена НЕТ scope `photos`. Фикс в vk.ts: wall media грузим через `ensureValidToken()` (user-токен админа) + fail-loud если медиа не загрузилось (не постим молча текст без фото). Сторис с фото работают (community-токен умеет stories, но не wall-photos). ЧТО НУЖНО: пере-авторизовать VK OAuth в Настройках, чтобы новый user-токен получил scope `photos` (конфиг уже просит `wall photos video stories offline`; старый токен выдан без photos); если после re-auth всё ещё error 15 → включить право `photos` в настройках VK-приложения / пройти модерацию VK. Refresh токена scope НЕ добавляет — нужен свежий consent. **ОБХОДНОЙ ПУТЬ (идея Юрия, TODO отдельной сессией):** постить VK фото/посты через **Postmypost** (как IG уже работает) — у Postmypost свой VK-апп с готовыми правами, минует scope `photos` и затянувшийся чат VK-поддержки. Нужно: подключить VK-канал в проекте Postmypost, проверить publication_type для VK + поддержку фото на стену, добавить роутинг VK→Postmypost (флаг `PlatformAccount.config.viaPostmypost` ИЛИ generic `PostmypostPublisher` по network); сторис VK уже работают напрямую — трогать не обязательно. **СТАТУС 2026-06-04:** Юрий подключает НаWоде VK-группу (SUP клуб НаWоде, gid 150371202) к Postmypost — выданы права: **Фотографии сообщества + Стена (управление)** + Истории + ЛС + Файлы + Сообщества + Уведомления. Т.е. фото+стена ЕСТЬ → постинг фото на стену через Postmypost пойдёт (минует наш scope-блокер). Осталось (отдельной сессией): взять PMP account_id VK-канала из проекта Postmypost + роутинг VK→Postmypost в коде.
...
- [2026-06-04] **Рефактор редактора (Эпик A — Фаза 4, per-channel превью + master/override):** Backend `PUT/DELETE /post-versions/:id` (правка/сброс оверрайда; 409 если PUBLISHED/SCHEDULED; FAILED→DRAFT при правке) +5 тестов. Frontend: превью-компоненты `components/posts/preview/{PostPreview,VkPreview,TelegramPreview,InstagramPreview}` + `usePlatformRegistry` (platformNote). PostEditorView «Настроить» (⚙) = превью «как в ленте» + **редактируемый per-channel оверрайд** (ленивое create/update через autosave debounce 1.2s, «Сбросить к мастеру»). `effectiveText` приоритет: живой черновик → версия-оверрайд → мастер (превью+счётчик live при наборе). `refreshVersions()` вместо loadPost (сохраняет выбор/раскрытие). `publishOneChannel` (ensure+publish 1 канал). Flush черновика ПЕРЕД publish/schedule/route-leave (awaited — не теряем правку; фикс из код-ревью). SCHEDULED/PUBLISHED оверрайд read-only. 143 теста. Задеплоено. Эпик A Фазы 1-5 готовы; единый канвас + полировка UI — позже.
...
- Security: resource-access, CSRF X-Tab-ID, rate limiting, path traversal, Zod, 143 теста
...
- Testing: Vitest, 143 tests, 11 files. Mock Prisma via vi.hoisted()
