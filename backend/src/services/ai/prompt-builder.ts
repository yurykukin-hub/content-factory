...
- ОБЯЗАТЕЛЬНО про запись/бронь: направляй в ШАПКУ ПРОФИЛЯ («запись — по ссылке в шапке профиля» / «бронь — в шапке профиля»). В IG ссылка в тексте НЕ кликабельна, поэтому НЕ пиши URL вроде nawode.ru — только «ссылка в шапке профиля».
  `.trim(),
}
...
/**
 * Промпт описания фото для каталога медиатеки под СЕМАНТИЧЕСКИЙ ПОИСК (Ф0).
 * Единый источник: используется ручной кнопкой describe, авто-describe при загрузке и batch-прогоном.
 * Сплошной текст без markdown (lite-модель иначе лепит буллеты — мешает поиску).
 */
export function buildGalleryVisionPrompt(): { system: string; user: string } {
  return {
    system: 'Ты ассистент медиатеки SMM-агентства. Описываешь фото на русском, чтобы по описанию можно было найти кадр по смыслу. Никогда не отказывай — описывай только то, что реально видно, без домыслов.',
    user: 'Опиши это фото сплошным текстом на русском (2–4 предложения, БЕЗ списков, нумерации и markdown-разметки). Включи: что/кто на фото и действие; локацию и обстановку (вода, залив, закат, замок, лес, город, причал, сапборды/SUP); время суток и погоду; настроение; для какого SMM-поста подойдёт (продающий / вдохновляющий / отзыв / погода / обучающий). Только факты, видимые на снимке.',
  }
}
...
export interface DigestStrategistContext {
  ...
  platforms: string[]
  count: number
  promoBlock?: string     // действующие скидки из ERP (Фаза 3, '' если нет/выключено)
  feedMode?: boolean      // true — это пост В ЛЕНТУ (PHOTO, рубрика дня; НЕ дублировать утреннюю погодную сторис)
}
...
УЖЕ ПРЕДЛАГАЛОСЬ В ПОСЛЕДНИЕ ДНИ (это твои прошлые идеи — дай СЕГОДНЯ ДРУГИЕ темы, углы и рубрики, НЕ повторяй и не перефразируй эти):
${ctx.recentProposals}
${ctx.competitorBlock ? `\nЗАЛЕТЕВШИЕ ПОСТЫ КОНКУРЕНТОВ (черпай идеи/форматы, НЕ копируй):\n${ctx.competitorBlock}\n` : ''}${ctx.promoBlock ? `\n${ctx.promoBlock}\nМожешь предложить ОДИН пост-акцию — ТОЛЬКО если скидка стоящая и реально заведена (из блока выше). НЕ выдумывай акции/проценты/спеццены, которых там нет; слабую скидку (−10%) в отдельный пост не выноси.\n` : ''}
ДОСТУПНЫЕ КАНАЛЫ: ${ctx.platforms.join(', ') || 'VK'}

${RF_LAW_RULE}

${ctx.feedMode
  ? `ЗАДАЧА: предложи ${ctx.count} пост В ЛЕНТУ на сегодня. Формат СТРОГО PHOTO (НЕ STORIES — сторис на сегодня уже публикуется отдельно, не дублируй её). Тема — из рубрик дня (обучение/команда/маршруты/польза SUP/история места), дающая ЦЕННОСТЬ или историю. НЕ делай пост «про сегодняшнюю погоду» и не «приглашение на воду» — это уже закрыто утренней сторис; здесь нужен ДРУГОЙ угол. Тему подбирай так, чтобы РЕАЛЬНО было фото в галерее (люди на сапах, вода, закат, замок с воды, команда). Не повторяй темы из «уже предлагалось».`
  : `ЗАДАЧА: на основе погоды, броней, рубрик и сезона предложи ${ctx.count} идеи контента ИМЕННО на сегодня. ТОЛЬКО форматы PHOTO и STORIES — каждый пост идёт с фотографией. НЕ предлагай TEXT, REELS, CLIPS, VIDEO (строго запрещены). Хотя бы одна STORIES с погодой/приглашением. Темы подбирай так, чтобы под них РЕАЛЬНО было фото в галерее (люди на сапах, вода, закат, замок с воды, команда) — избегай абстрактных тем без визуала. ВАЖНО про разнообразие: чередуй рубрики и углы подачи — отдавай предпочтение рубрикам, которые НЕ встречаются в списке «уже предлагалось» выше; не повторяй формат/тему вчерашних предложений.`}

Для КАЖДОЙ идеи верни:
- rubric: рубрика из списка выше
- theme: конкретная тема/угол подачи (1 фраза)
- format: PHOTO | STORIES (ТОЛЬКО эти два — каждый пост обязательно с фотографией)
...
- photoKeywords: 2-4 РУССКИХ ключевых слова для поиска фото. ВАЖНО: включай сап-активность («сапборд», «сап», «на доске») — на фото нужны люди на SUP-досках (это ядро SUP-проката), плюс объект/локацию темы: ["сапборд","замок"], ["сап","закат"], ["сапборд","дети"]. Обязательно для всех предложений.
- reasoning: почему именно это сегодня (погода/бронь/рубрика, 1 фраза)

Ответь СТРОГО JSON без markdown:
{"ideas":[{"rubric":"...","theme":"...","format":"PHOTO|STORIES","channels":["VK","INSTAGRAM"],"keyMessage":"...","photoKeywords":["...","..."],"reasoning":"..."}]}`
}
...
Погоду описывай СЛОВАМИ (тепло, штиль, слабый/умеренный ветер) — НЕ указывай скорость ветра в цифрах или м/с (люди их не понимают). Температуру в °C можно.
...
ВЫЧИТКА (обязательно): перед ответом перечитай текст и исправь орфографию, опечатки и согласование слов (частые ошибки: «едут/едят», падежи, «тся/ться»). Текст должен быть грамотным и звучать естественно по-русски.
...
ЗАДАЧА: выбери id ОДНОГО фото, РЕАЛЬНО подходящего посту — объекты и локация на фото должны совпадать с темой (тема про замок → на фото должен быть замок; про закат → закат; про детей → дети). При прочих равных ПРЕДПОЧИТАЙ кадры, где видны ЛЮДИ НА SUP-ДОСКАХ (это SUP-прокат — активность важнее пустого пейзажа): для темы «замок» лучше «люди на сапах на фоне замка», чем просто замок. СТРОГО: если совпадения по содержанию нет — верни null. Нерелевантное фото ХУЖЕ отсутствия фото; НЕ подбирай «похожее по настроению».
...
// ─────────────────────────────────────────────────────────────────────────
// Ритм-движок (Ф2): ОДНА сторис под роль времени дня (утро/день/вечер).
// Генерится В СВОЁ ВРЕМЯ по свежим данным — не пачкой утром (иначе к вечеру протухло).
// ─────────────────────────────────────────────────────────────────────────

export type DigestStoryRole = 'morning' | 'day' | 'evening'

export interface DigestRoleStoryContext {
  dayName: string
  dateStr: string
  brandContext: string
  seasonHint: string
  rubrics: string[]
  dataBlock: string       // погода + брони
  hotSlotsBlock: string   // «горячие» слоты (где уже есть записанные)
  recentProposals: string
  platforms: string[]
...
/**
 * Промпт ОДНОЙ сторис под роль времени дня. Один Sonnet-вызов делает и стратегию,
 * и копию (для сторис per-platform адаптация не нужна — одна вертикальная картинка).
 */
export function buildDigestRoleStoryPrompt(role: DigestStoryRole, ctx: DigestRoleStoryContext): string {
  const roleBlock = role === 'morning'
    ? `СЕЙЧАС УТРО. Роль сторис — задать тон дня: доброе утро + сегодняшняя погода (словами) + лёгкое приглашение на воду. Если в «горячих слотах» есть что-то на сегодня — мягко упомяни. CTA — записаться/забронировать.`
    : role === 'day'
    ? `СЕЙЧАС ДЕНЬ. Роль сторис — СЛОТ-ФИЛЛ (добивка). Возьми ОДИН горячий слот из списка ниже и позови присоединиться: «уже N человек на [тур] в [время] — поедем точно, присоединяйся». Если у слота в данных известно и мало свободных мест — добавь «осталось ~M мест» (НЕ выдумывай число, бери из данных; нет числа — не пиши про места). Если горячих слотов нет — сделай сторис «прямо сейчас на воде» по сегодняшней погоде. CTA — забронировать.`
    : `СЕЙЧАС ВЕЧЕР. Роль сторис — рекап + завтра: тёплый итог дня и приглашение на завтра (погода завтра словами + что есть свободные места/слоты). «Завтра ждём на воде». CTA — забронировать на завтра.`

  return `Ты — SMM-контент-стратег и копирайтер НаWоде. Сегодня ${ctx.dayName}, ${ctx.dateStr}.
Делаешь ОДНУ сторис (вертикальную 9:16) ИМЕННО на этот момент дня.

${ctx.brandContext}

СЕЗОН: ${ctx.seasonHint}
${ctx.rubrics.length ? `\nРУБРИКИ (выбери одну в поле rubric):\n${ctx.rubrics.map(r => `- ${r}`).join('\n')}` : ''}

ПОГОДА И БРОНИ:
${ctx.dataBlock}

ГОРЯЧИЕ СЛОТЫ (где УЖЕ есть записанные — для слот-филла):
${ctx.hotSlotsBlock}
${ctx.promoBlock ? `\n${ctx.promoBlock}\n` : ''}
УЖЕ ПРЕДЛАГАЛОСЬ (НЕ повторяй темы, формулировки И ФОРМАТ заголовка — если в примерах ниже заголовок начинается с дня недели и градусов или содержит слово «температура», ты так НЕ делай, это плохой образец):
${ctx.recentProposals}

${roleBlock}
${ctx.promoBlock ? '\nПРО СКИДКИ (ВАЖНО): в блоке «ДЕЙСТВУЮЩИЕ СКИДКИ» выше есть реальная скидка — ОБЯЗАТЕЛЬНО впиши её в текст как повод записаться именно сейчас (1 фразой: на что и сколько процентов/цена). Бери проценты/цены СТРОГО из блока — другие акции НЕ выдумывай. Без кликбейта и давления, тоном бренда. (Если блок выше пуст — про скидки молчи.)\n' : ''}
${RF_LAW_RULE}
Погоду — СЛОВАМИ (тепло, штиль, слабый ветер), без цифр скорости ветра. Температуру в °C можно.
Текст сторис — КОРОТКИЙ (1-3 фразы), как подпись на картинке. Перечитай на грамотность/опечатки (без «едут/едят» и т.п.). В сторис хэштеги НЕ нужны (hashtags оставь пустым), в поле text НЕ пиши символ #.
theme (это ЗАГОЛОВОК НА КАРТИНКЕ) — живой и короткий (3-6 слов), про погоду/настроение/приглашение на воду. НЕ строй его вокруг температуры: БЕЗ градусов и БЕЗ слова «температура» (виджет сверху уже показывает погоду; «идеальная температура» звучит сухо). Хорошо: «Сегодня идеальная погода для SUP», «Выборг зовёт на воду», «Лучшее утро — на доске», «Первое знакомство с сапом». Плохо: «Вторник +20°C — идеальная температура».
photoKeywords — 2-4 русских слова для поиска РЕАЛЬНОГО фото в галерее (обязательно «сап»/«сапборд» + объект/локация темы).

Ответь СТРОГО JSON без markdown:
{"rubric":"...","theme":"короткая тема","text":"текст сторис без хэштегов","photoKeywords":["сап","..."],"hashtags":[],"reasoning":"почему именно сейчас (1 фраза)"}`
}

export interface FlashStoryContext {
  dayName: string
  dateStr: string
  brandContext: string
  seasonHint: string
  weatherLine: string       // погода сегодня словами (для хука)
  percent: number           // глубина скидки (уже обрезана красной линией)
  serviceLabel: string      // на что скидка (прокат)
}

/**
 * Промпт погодного FLASH-сторис (Фаза 3, этап 2): разовая скидка «только сегодня», когда
 * распогодилось и есть свободные места. Процент и услуга — СТРОГО из сигнала (не выдумывать).
 */
export function buildFlashStoryPrompt(ctx: FlashStoryContext): string {
  return `Ты — SMM-копирайтер НаWоде. Сегодня ${ctx.dayName}, ${ctx.dateStr}. Делаешь ОДНУ flash-сторис (вертикаль 9:16).

${ctx.brandContext}

СЕЗОН: ${ctx.seasonHint}
ПОГОДА СЕГОДНЯ: ${ctx.weatherLine}

ПОВОД: распогодилось, а места на сегодня ещё есть — ловим момент разовой скидкой.
СКИДКА (бери ТОЧНО эти значения, НЕ меняй и не выдумывай другие): −${ctx.percent}% на ${ctx.serviceLabel}, ТОЛЬКО СЕГОДНЯ.

ЗАДАЧА: цепкая короткая сторис (1-3 фразы) — солнце/погода зовёт на воду + «только сегодня −${ctx.percent}% на ${ctx.serviceLabel}» + срочность (момент уйдёт). Без давления и кликбейта, тёплый тон бренда.

${RF_LAW_RULE}
Погоду — СЛОВАМИ. Хэштеги НЕ нужны (hashtags пустой), в text символ # НЕ пиши. Упоминай ТОЛЬКО эту скидку (−${ctx.percent}% на ${ctx.serviceLabel}); другие проценты/услуги не придумывай.
theme (ЗАГОЛОВОК НА КАРТИНКЕ) — живой, короткий (3-6 слов), про момент/приглашение. БЕЗ градусов и слова «температура» (виджет уже показывает погоду). Хорошо: «Лови погоду на воде», «Сегодня идеальный день для SUP».
photoKeywords — 2-4 русских слова для поиска РЕАЛЬНОГО фото (обязательно «сап»/«сапборд» + солнечная вода/лето).

Ответь СТРОГО JSON без markdown:
{"theme":"короткая тема","text":"текст flash-сторис без хэштегов","photoKeywords":["сап","..."],"hashtags":[],"reasoning":"почему flash сейчас (1 фраза)"}`
}

/**
 * Промпт ВК-поста о наборе инструкторов (еженедельно, ~среда). Только VK (не Instagram).
 * Вакансия = живой пост, не реклама товара; CTA — куда откликаться (contact).
 */
export function buildDigestRecruitmentPrompt(
  ctx: { dayName: string; dateStr: string; brandContext: string; seasonHint: string },
  contact: string,
): string {
  return `Ты — HR + SMM НаWоде. Напиши пост для ВКонтакте о наборе инструкторов SUP в команду на сезон.
Сегодня ${ctx.dayName}, ${ctx.dateStr}. Это пост ТОЛЬКО для ВК (не Instagram).

${ctx.brandContext}

СЕЗОН: ${ctx.seasonHint}

ЗАДАЧА: тёплый живой пост-вакансия (не сухое объявление). Структура:
- Хук: зови в команду («Ищем инструкторов в команду НаWоде» / «Проведи лето на воде, а не в офисе»).
- Кто нужен: любит воду и активный отдых, ладит с людьми, ответственный. ВАЖНО подчеркнуть — МОЖНО БЕЗ ОПЫТА, всему научим.
- Что даём: гибкий график, сезонная подработка/работа, обучение, классная команда, бесплатный прокат/туры в свободное время, Выборг (набережная).
- CTA: как откликнуться — ${contact}.

${RF_LAW_RULE}
Тон дружелюбный, без канцелярита. 600-1000 символов. В поле text НЕ пиши хэштеги и символ # (хэштеги — только в массиве hashtags, без #).
photoKeywords — 2-4 русских слова для поиска фото в галерее (команда / инструктор / люди на сапах / на воде).

Ответь СТРОГО JSON без markdown:
{"theme":"короткая тема","text":"текст поста БЕЗ хэштегов","photoKeywords":["команда","сап"],"hashtags":["НаWоде","Выборг","работа","вакансия","SUP"]}`
}
...
export function buildPostPrompt(
  ...
  parts.push('')
  parts.push('Напиши только текст поста, без пояснений.')
  return parts.join('\n')
}
