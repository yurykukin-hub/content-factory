...
- **Кадр дизайн-сторис (2026-06-20):** `buildStoryDesign` принимает `photoPosition` (objectPosition — satori это поддерживает). Модалка `StoryDesignModal.vue` (ползунок вертикального фокуса + живое CSS-превью = satori + правка заголовка → перезапекание `render-design`). Кнопка «Поправить кадр» в дайджесте и редакторе. `MediaFile.sourceMediaId` хранит исходное фото → переоформление берёт оригинал (не дизайн-поверх). Модалка публикации — галочки каналов (VK/IG Stories) + результат по каждому
