...
/**
 * Instagram Publisher — публикация через сервис Postmypost (postmypost.io).
 *
 * Почему через Postmypost, а не напрямую в Meta Graph API:
 *  - официальный Instagram API из РФ требует Business + FB Page + App Review;
 *  - Postmypost берёт это на себя, отдаёт REST API.
 *
 * Реализация — прямые fetch к REST (как vk.ts), без SDK (его typings сломаны).
 *
 ...
 *
 * Доступы в PlatformAccount: accessToken=токен, accountId=postmypost account_id,
 * config.postmypostProjectId=project_id. Fallback .env (POSTMYPOST_API_TOKEN/PROJECT_ID).
 */
...
export class InstagramPublisher implements Publisher {
  ...
  async publish(params: PublishParams): Promise<PublishResult> {
    ...
    const accountId = Number(platformAccount.accountId)
    if (!Number.isFinite(accountId)) return { success: false, error: `Некорректный Postmypost account_id: "${platformAccount.accountId}"` }
    if (!mediaFiles?.length) return { success: false, error: 'Instagram-публикация требует хотя бы один медиафайл.' }
  ...
  private async uploadMedia(token: string, projectId: number, mf: { url: string; mimeType: string; filename: string }): Promise<number | null> {
    ...
    if (!init.ok || !init.data?.id || !init.data?.action) {
      console.error('[IG] upload/init failed:', init.status, JSON.stringify(init.data))
      return null
    }
    ...
    if (!done.ok) {
      console.error('[IG] upload/complete failed:', done.status, JSON.stringify(done.data))
      return null
    }
