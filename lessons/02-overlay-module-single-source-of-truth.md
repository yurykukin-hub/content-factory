...
1. ~~`cleanStoryTitle` срезает день недели в ручном тексте~~ **✅ ИСПРАВЛЕНО 03.07** (`2202f17`): `buildStoryDesign` получил флаг `cleanTitle` (default true = авто-дайджест чистит день/температуру); `render-overlay` (ручной путь) шлёт `cleanTitle:false` → текст пользователя сохраняется дословно (эмодзи всё равно стрипаем — satori). Проверено: ручной бейк держит «Пятница»/«Суббота», дайджест по-прежнему чистит.
