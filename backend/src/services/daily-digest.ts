...
import { detectRouteFolder, searchGalleryPhotos, getRecentlyUsedPhotoIds } from './digest/photo-search'
...
// detectRouteFolder / searchGalleryPhotos / getRecentlyUsedPhotoIds → ./digest/photo-search
 ...
 */
export async function approveDigestTask(task: any, opts?: { keepProposed?: boolean }): Promise<{ postId: string; postType: string }> {
  ...
  // Идемпотентность: задача уже связана с черновиком (открывалась «В редактор») — переиспользуем его,
  // не плодя дубль Post. При обычном одобрении/публикации переводим в approved.
  if (task.postId) {
    const existing = await db.post.findUnique({ where: { id: task.postId }, select: { id: true, postType: true } })
    if (existing) {
      if (!opts?.keepProposed) {
        await db.autoPostTask.update({ where: { id: task.id }, data: { status: 'approved', decidedAt: new Date() } })
      }
      return { postId: existing.id, postType: existing.postType }
    }
  }
  ...
  await db.autoPostTask.update({
    where: { id: task.id },
    // keepProposed: «В редактор» из дайджеста создаёт Post, но предложение ОСТАЁТСЯ видимым в дайджесте (proposed).
    data: opts?.keepProposed
      ? { postId: post.id }
      : { status: 'approved', postId: post.id, decidedAt: new Date() },
  })

  emitEvent({ type: 'post_created', tabId: '', postId: post.id })
  log.info('[Digest] approved -> draft post', { taskId: task.id, postId: post.id, postType: pt, photo: !!task.mediaFileId, keepProposed: !!opts?.keepProposed })
