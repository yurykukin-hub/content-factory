...
- **C3-долги (оставлены намеренно):** видео-модели (живой каталог KIE + 1-2 модели per-model; колонка `model` есть, фронт не шлёт, роут не персистит); полиш-бэклог (layout Sound чат/композер, welcome-строка «Настройки:…», иконки в `SharedEnhanceMenu`, Video pro-debug-readout); баг edit-image (`ImageEditModal`→`submitted`, MediaLibrary/MediaUpload слушают `@edited` → «Применить» не зовёт API; StoryEditor ок); 2 падающих бэк-теста (`digest/__tests__/photo-search.test.ts`).
