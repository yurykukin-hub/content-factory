...
## Блокеры
- [2026-06-04] VK scope `photos` застрял в VK-поддержке → ОБОЙДЁН через Postmypost (фото/стена). scope `stats` (для метрик) — нужна ре-авторизация Юрия в Настройках CF.
