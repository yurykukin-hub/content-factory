...
/**
 * Session list for Photo Studio.
 * Pattern from SsSessionBar.vue, adapted for photo generation.
 */
import { ref, nextTick } from 'vue'
import { Plus, Pencil, Trash2, Check, X, Camera, Loader2, ChevronDown } from 'lucide-vue-next'
...
function startRename(s: PhotoSession, e: Event) {
  e.stopPropagation()
...
function resultCount(s: PhotoSession): number {
  if (!Array.isArray(s.results)) return 0
  ...
  <div class="flex flex-col min-h-0">
    ...
    <div :class="[
      ...
      <div v-for="s in sessions" :key="s.id"
        ...
        <!-- Thumbnail (first result or icon) -->
        <div class="w-9 h-9 rounded-lg overflow-hidden bg-gray-100 dark:bg-gray-800 shrink-0 flex relative items-center justify-center">
          ...
          <img v-if="s.results?.length && s.results[0].thumbUrl"
            :src="s.results[0].thumbUrl" class="w-full h-full object-cover" />
          <Camera v-else :size="14" class="text-gray-300" />
        </div>

        <!-- Info -->
        <div class="flex-1 min-w-0">
          <div class="flex items-center gap-1.5">
            <span :class="['w-1.5 h-1.5 rounded-full shrink-0', STATUS_DOT[s.status] || 'bg-gray-400']" />
          ...
          <div class="flex items-center gap-1.5 text-[9px] text-gray-400 mt-0.5">
            ...
            <span v-if="s.photoModel">· {{ s.photoModel === 'nano-banana-pro' ? 'NB Pro' : 'NB2' }}</span>
            <span v-if="resultCount(s)">· {{ resultCount(s) }} фото</span>
          </div>
        ...
        <div v-if="confirmDeleteId === s.id" class="flex items-center gap-0.5 shrink-0" @click.stop>
          <button @click="emit('delete', s.id)" class="p-1 rounded text-red-500 hover:bg-red-500/10"><Check :size="14" /></button>
