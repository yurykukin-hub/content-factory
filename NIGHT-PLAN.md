...
- [x] **A4** Музыка в сторис: `overlayAudioOnVideo()` (ffmpeg мукс), `/media/overlay-video` принимает `musicSessionId`/`audioMediaFileId`, picker треков из Sound Studio в редакторе, музыка вшивается в видео ("bake once"), persist в publishOptions. ✅ DEPLOYED
...
- [x] **B3 (UI)** Планирование любого поста из PostEditorView: datetime + «В план» на версию канала, баннер запланированного + отмена, «Опубликовать сейчас». ✅ DEPLOYED. Драфты агента можно планировать. (Scheduler бэкенд уже публиковал все типы.)
- [x] **B2** Выбор/смена типа поста в PostEditorView (TEXT/PHOTO/VIDEO/REELS/CLIPS) + format-hint + предупреждение «нужно видео». ✅ DEPLOYED
- [x] **B4** IG Reels: бэк (Postmypost type=4) + UI-валидация (9:16, ≤90с hint, видео обязательно). ✅
- [x] **B5** ВК Клипы: `PostType.CLIPS` (миграция enum). VK публикует как вертикальное видео (video.save+wall.post — публичного API «Клипов» нет, задокументировано). IG: CLIPS→Reels(4). TG: видео. ✅
- [x] **B6** TG видео: проверено — telegram.ts шлёт video по mimeType (sendVideo). ✅

### Эпик D — Интеллект плана ✅
- [x] **История публикаций в контекст:** `buildPostPrompt` принимает recentPosts; ai-generate/generate-all передают последние 8 постов («не повторяйся»). ✅
- [x] **Рубрики:** общий модуль `nawode-strategy.ts` (10 рубрик + сезон), generate-plan для НаWоде подставляет рубрики по умолчанию; daily-digest рефакторен на общий источник. ✅
- [x] **Regenerate ячейки с направлением:** `POST /plan-items/:id/regenerate` (Haiku, учитывает рубрику+недавние посты) + кнопка «переделать» в ContentPlansView (window.prompt направления). ✅
- [x] **Форматы в плане:** generate-plan/import допускают REELS/CLIPS/STORIES (раньше только TEXT/PHOTO/VIDEO). ✅
- [~] ERP-события (туры/праздники) автоматически в план — частично (сезонный контекст есть; авто-импорт броней в план как отдельных поводов не делал). Метрики/обратная связь — не делал (нет данных охватов).
