...
- [x] **Метрика подключена + цель-конверсия настроена** (автономно через Management API): визиты по utm (262 виз/30д) + goalIds=[302969632] «Зашли на РАСПИСАНИЕ» (booking-intent). SMM-ROI замкнут end-to-end. Точной цели «бронь» в Метрике нет — реальные брони = ERP-контракт.
